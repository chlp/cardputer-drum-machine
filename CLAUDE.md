...
- **Audio**: runs on a dedicated FreeRTOS task pinned to core 0 (`audioTaskFn`), priority 2, guarded by a mutex. The main loop is never blocked by decoding. `startMp3` / `stopAudio` take the mutex, swap `gen`/`src`, and release. `audioEndedNaturally` flag is set by the task; handled in `loop()`. The decoder loop ends with `vTaskDelay(1)` so lower-priority tasks get CPU.
- **Watchdog**: the audio task pinned to core 0 will starve IDLE0 — that's the whole point of giving it a dedicated core. The default IDF task watchdog watches IDLE0 and would `abort()` after 5 s (call site: `task_wdt_isr` in `panic.c`, manifests as random reboots during long MP3 playback). Setup calls `disableCore0WDT()` and the audio task subscribes itself with `esp_task_wdt_add(nullptr)` + `esp_task_wdt_reset()` per loop iteration, so we still catch a hung decoder (e.g. libmad inf-loop on a corrupt frame) without false positives. **Do not re-enable IDLE0 WDT.**
...
- **Audio task stack**: 20 KB. libmad needs ~10–12 KB for stereo MP3 frames; some VBR / joint-stereo files push higher. Watch the `[AUDIO] task_stack_min_free=…` log line — bump `xTaskCreatePinnedToCore` in `audio.cpp` if it drops below ~2 KB.
- **Logging**: `src/log.h`/`log.cpp` exposes `logLine(tag, fmt, …)`, `logBoot()`, `logHeap(tag)`, `logTaskStack(tag, h)`. Output format: `[<millis>][TAG] message`. The leading `[…][…]` matches the regex in `tools/sd_xfer.py` so SD-over-serial transfers are not disrupted. `logBoot()` prints `esp_reset_reason()` — that line is the first thing to look at after a spontaneous reboot.
